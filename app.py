...
def feed():
    ...
    for item in context["daily_report"]:

        if item.get("priority") in ["CRITICAL", "HIGH"]:
            feed_items.append({
                "level": "CRITICAL",
                "message": f'{item["cve"]} scored {item["threat_score"]}'
            })

        if item.get("threat_references"):

            for ref in item["threat_references"]:

                feed_items.append({
                    "level": "HIGH",
                    "message": f'{ref} linked to {item["cve"]}'
                })
